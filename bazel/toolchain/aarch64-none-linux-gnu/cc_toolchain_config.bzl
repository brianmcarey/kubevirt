...
This file defines the C++ toolchain for cross-compiling to aarch64 (ARM64) architecture.

Architecture: aarch64 (ARM64)
Tools: aarch64-linux-gnu-* (GCC cross-compiler toolchain)
OS Support: CentOS Stream 9 (GCC 12.x)

Related Files:
  - cc_toolchain_config_cs10.bzl: CentOS Stream 10 variant (GCC 14.x)
  - ../s390x-none-linux-gnu/cc_toolchain_config.bzl: IBM System z variant
...
Note: This file is similar to s390x and x86_64 variants. They are separate files
...
def _impl(ctx):
    # Tool paths for cross-compilation to aarch64.
    # These point to the aarch64-linux-gnu-* cross-compiler tools installed on the build machine.
    # Path: /usr/bin/aarch64-linux-gnu-* (assumed to be in PATH or fully qualified)
    tool_paths = [
        tool_path(
            name = "ar",
